...
int ed25519_key_generate(uint8_t public_key[], uint8_t private_key[])
{
    ...
    WC_RNG rng;
    err = wc_ed25519_make_key(&rng, ED25519_KEY_LENGTH, &ed25519_key);
    if (err < 0) {
        ESP_LOGE(TAG, "wc_ed25519_make_key failed. err:%d\n", err);
        return -1;
    }

    int key_length = ED25519_KEY_LENGTH;
    wc_ed25519_export_public(&ed25519_key, (byte*)public_key, (word32*)&key_length);

    key_length = ED25519_KEY_LENGTH;
    wc_ed25519_export_private(&ed25519_key, (byte*)private_key, (word32*)&key_length);
...
{
    ed25519_key ed25519_key;
    wc_ed25519_init(&ed25519_key);
    wc_ed25519_import_private_key(private_key, ED25519_KEY_LENGTH, public_key, ED25519_KEY_LENGTH, &ed25519_key);
    wc_ed25519_sign_msg(in, in_len, signatured, (word32*)signatured_len, &ed25519_key);
    return 0;
}
